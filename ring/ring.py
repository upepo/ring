...
class ModelRing(Ring):
    ...
    def __init__(self, model, keys, storage=None):
    ...
    def __call__(self, f):
        def get_or_update(_self, *args, **kwargs):
            ...
            built_args['_tag'] = _build_func_key(f, args, kwargs)
            func_args = {k: v for k, v in built_args.items() if k not in self.model.key_mapper.values()}
            print(func_args)
            return self.get_or_update(f, **func_args)
        return get_or_update
